...
template<typename T>
void
readTable(std::istream &is, size_t tableOffset, size_t entryCount, const char *tableKeyString, std::vector<T> &output) {
    // Read data
        ...
        if (fileKey == 0xFFFFFFFF) {
            do {
                if (blockEntry->fileFlag & (BLOCK_COMPRESSED | BLOCK_IMPLODED)) {
                    const uint32_t offsetTableLength = encryptedOffsetTable.size() * 4;

                    // Get decryption key
                    fileKey = GetFileDecryptKey(
                            encryptedOffsetTable.data(),
                            offsetTableLength,
                            offsetTableLength,
                            [&](const void *_decrypted) {
                                const auto decryptedOffsetTable = static_cast<const uint32_t *>(_decrypted);
                                // Last table
                                if (decryptedOffsetTable[0] != offsetTableLength) return false;
                                if (decryptedOffsetTable[sectorNum] != blockEntry->blockSize) return false;
                                bool valid = true;
